...
import "../structures/BurnInfo.sol";
...
abstract contract AFlexClient {
    ...
    function deployPriceXchg(
        ...
        uint256 user_id,
        uint256 order_id
    ) public functionID(0xa) returns(address value0) {}
    ...
    function destroyIndex(uint256 user_id, uint128 evers) public functionID(0x11) {}

    function burnThemAll(uint128 burn_ev, BurnInfo[] burns) public functionID(0x13) {}
    function cancelThemAll(uint128 cancel_ev, address[] prices) public functionID(0x14) {}

    function unwrapWallet(
        uint128 evers_value,
        uint256 out_pubkey,
    ...
    function bindWallet(
        ...
        bool set_trader,
        optional(uint256) trader
    ) public functionID(0x16) {}

    function upgrade(uint128 request_evers, address user_data_cfg) public functionID(0x17) {}

    function getPayloadForDeployInternalWallet(uint256 owner_pubkey, optional(address) owner_addr, uint128 evers, uint128 keep_evers) public functionID(0x18) returns(TvmCell value0) {}

    function getDetails() public functionID(0x1c) returns(
        uint256 owner,
        FlexVersion triplet,
        optional(FlexVersion) ex_triplet,
        TvmCell auth_index_code,
        TvmCell user_id_index_code
    ) {}

    function burnWallet(uint128 evers_value, uint256 out_pubkey, optional(address) out_owner, address my_tip3_addr, optional(TvmCell) notify)public functionID(0x12) {}

    function getPriceXchgAddress(uint128 price_num, TvmCell salted_price_code) public functionID(0x1a) returns(address value0) {}

    function transfer(address dest, uint128 value, bool bounce) public functionID(0xc) {}
